...
int dummy_pos3; //posisi dummy 1-6
int dummy_pos4; //posisi dummy 7-8
//------
...
void Parsing_data2(char input[],int &out1,int &out2,int &out3,int &out4,int &out5,int &out6,int &out7,int &out8,int &out9,int &out10,int &out11,int &out12,int &out13,int &out14,int &out15,int &out16,int &out17,int &out18,int &out19,int &out20,int &out21,int &out22,int &out23,int &out24,int &out25,int &out26,int &out27,int &out28,int &out29,int &out30,int &out31,int &out32,int &out33, int &out34, int &out35)//,int &out34,int &out35,int &out36,int &out37,int &out38,int &out39,int &out40,int &out41)//,int &out42,int &out43,int &out44,int &out45,int &out46,int &out47)//,int &out48,int &out49,int &out50,int &out51)
{
    pry2 = strtok(input, ",");
...
striker_3.angle_ball[1], mode_formasi[0], striker_3.x_cm[0], striker_3.y_cm[0], 
striker_2.x_cm[0], striker_2.y_cm[0], timer2, timer3);
			...
			if (awal == 12)
			{
				...
				{			
			   		striker_2.x_or[0] = nilai_x;							// odometry x robot
								...
								striker_2.x_or[2] = 1;							// odometry x bola
				...
				if (TEAM == CYAN)
				{
					...
					if(count_ > 15){
						if(count_send % counter_S2_2 == 0){
							...
							fprintf(serial_send, "90,%d,%d,%d,%d,%d\n", dummy_pos1, dummy_pos2, dummy_pos3, dummy_pos4, timer3);}
				...
				if (TEAM == CYAN)
				{
					...
					if(count_ > 15){
						if(count_send % counter_S3_2 == 0){
							...
							fprintf(serial_send, "90,%d,%d,%d,%d,%d\n", dummy_pos1, dummy_pos2, dummy_pos3, dummy_pos4, timer2);}
...
//				striker_3.y_ball[1] = striker_3.y_or[2];
			...
			if (PROGRAM_UNTUK == STRIKER_2)
			{
				sprintf(buff_send, "111,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n", striker_2.x_or[1], striker_2.y_or[1], (int)striker_2.distance_ball[1], (int)striker_2.angle_ball[0], striker_2.x_or[2], striker_2.y_or[2], striker_2.yaw[1], (int)striker_2.x_mk_pos, (int)striker_2.y_mk_pos, striker_2.Volt[1], (int)striker_2.timer);
				sendto(s, buff_send, strlen(buff_send), 0, (struct sockaddr *) &si_other, slen);
				memset(buff_send,'\0', sizeof(buff_send));	
			}
			else if (PROGRAM_UNTUK == STRIKER_3)
			{
				sprintf(buff_send, "222,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n", striker_3.x_or[1], striker_3.y_or[1], (int)striker_3.distance_ball[1], (int)striker_3.angle_ball[0], striker_3.x_or[2], striker_3.y_or[2], striker_3.yaw[1], (int)striker_3.x_mk_pos, (int)striker_3.y_mk_pos, striker_3.Volt[1], (int)striker_3.timer);
				sendto(s, buff_send, strlen(buff_send), 0, (struct sockaddr *) &si_other, slen);
				memset(buff_send,'\0', sizeof(buff_send));	
			}
			else if (PROGRAM_UNTUK == KEEPER)
