...
// #define SERVER "10.26.56.180"
// #define SERVER "192.168.43.18"
#define SERVER "192.168.0.101"
#define BUFLEN 512  //Max length of buffer
#define PORT 8080   //The port on which to send data

static const char* portName = "/dev/video0";		//TTL BIRU
...
int dummy_pos3=0; //posisi dummy 4-6
int dummy_pos4=0; //posisi dummy 7-8
int timer2=0;
int timer3=0;
int dummypos1, dummypos2, dummypos3, dummypos4, flagKirim;
int flag_striker,flagStriker;
int flagData1, flagData2, flagData3, flagData4 = 1;
//------
...
void Parsing_data2(char input[],int &out1,int &out2,int &out3,int &out4,int &out5,int &out6,int &out7,int &out8,int &out9,
int &out10,int &out11,int &out12,int &out13,int &out14,int &out15,int &out16,int &out17,int &out18,int &out19,int &out20,
int &out21,int &out22,int &out23,int &out24,int &out25,int &out26,int &out27,int &out28,int &out29,int &out30,int &out31,
int &out32,int &out33, int &out34, int &out35)//,int &out34,int &out35,int &out36,int &out37,int &out38,int &out39,int &out40,int &out41)//,int &out42,int &out43,int &out44,int &out45,int &out46,int &out47)//,int &out48,int &out49,int &out50,int &out51)
{
    pry2 = strtok(input, ",");
    while (pry2 != 0 && pcount <= 35)
    {
...
Parsing_data2(buff_rcv, X, Y, Z, 
striker_3.coor_x[0], striker_3.coor_y[0],
striker_2.coor_x[0], striker_2.coor_y[0], 
flag_wifi, 
striker_3.coor_x[2], striker_3.coor_y[2], 
striker_2.coor_x[2], striker_2.coor_y[2], 
dummy_pos1, dummy_pos2, dummy_pos3, dummy_pos4,
striker_2.yaw[0], striker_3.yaw[0],
striker_2.distance_ball[0], striker_3.distance_ball[0], 
striker_3.reset_x[0], striker_3.reset_y[0], 
striker_2.reset_x[0], striker_2.reset_y[0], 
TEAM, mode_marking, 
striker_2.angle_ball[1], striker_3.angle_ball[1], 
striker_3.x_cm[0], striker_3.y_cm[0], 
striker_2.x_cm[0], striker_2.y_cm[0], 
timer2, timer3, flagStriker);
...
//		if(buff_rcv != 0) printf("DATA = %s\n", buff_rcv);
		...
		flagKirim = flag_wifi;
		dummypos1 = dummy_pos1;
		dummypos2 = dummy_pos2;
		dummypos3 = dummy_pos3;
		dummypos4 = dummy_pos4;
		// flagStriker = flag_striker;
        // flagStriker = 5;
...
//			printf("S3---->>>%s", rcv_mk);
			Parsing_data(rcv_mk, awal, nilai_x, nilai_y, rawTimer, nilai_y1, rawYaw, Volt, Proxi);
...
//			else striker_2.reset_yaw[1] = 0;
			...
			if(striker_3.coor_x[1] < x_BS_min[1]) 
					// striker_2.y_mk_pos = (striker_2.coor_x[1] - penambah_D[0])/pengali_B[0];
					striker_3.y_mk_pos = (pengali_B[0]*striker_2.coor_x[1]) + penambah_D[0];

			else if(striker_3.coor_x[1] >= x_BS_min[1] && striker_2.coor_x[1] <= x_BS_max[1]) 
					// striker_2.y_mk_pos = (striker_2.coor_x[1] - penambah_D[1])/pengali_B[1];
					striker_3.y_mk_pos = (pengali_B[1]*striker_2.coor_x[1]) + penambah_D[1];
			else if(striker_3.coor_x[1] > x_BS_max[1]) 
					// striker_2.y_mk_pos = (striker_2.coor_x[1] - penambah_D[2])/pengali_B[2];
					striker_3.y_mk_pos = (pengali_B[2]*striker_2.coor_x[1]) + penambah_D[2];

			if(striker_3.coor_y[1] < y_BS_min[1]) 
					// striker_2.x_mk_pos = (striker_2.coor_y[1] - penambah_C[0])/pengali_A[0];
					striker_3.x_mk_pos = (pengali_B[0]*striker_3.coor_y[1]) + penambah_D[0];
			else if(striker_3.coor_y[1] >= y_BS_min[1] && striker_3.coor_y[1] <= y_BS_max[1]) 
					// striker_2.x_mk_pos = (striker_2.coor_y[1] - penambah_C[1])/pengali_A[1];
					striker_3.x_mk_pos = (pengali_B[1]*striker_3.coor_y[1]) + penambah_D[1];
			else if(striker_3.coor_y[1] > y_BS_max[1])
					// striker_2.x_mk_pos = (striker_2.coor_y[1] - penambah_C[2])/pengali_A[2];
					striker_3.x_mk_pos = (pengali_B[2]*striker_3.coor_y[1]) + penambah_D[2];
				...
				if (TEAM == CYAN)
				{
					...
					if(count_ > 15){
						if(count_send % counter_S2_2 == 0){
							fprintf(serial_send, "13,%d,%d,%d,%d,%d\n",(int)striker_2.x_mk_pos, (int)striker_2.y_mk_pos, (int)striker_3.coor_x[1], (int)striker_3.coor_y[1], flagKirim);
							fprintf(serial_send, "90,%d,%d,%d,%d,%d\n", dummypos1, dummypos2, dummypos3, dummypos4, timer3);
							// int Bytesend = fprintf(serial_send, "90,%d,%d,%d,%d,%d\n", dummypos1, dummypos2, dummypos3, dummypos4, timer3);
							// printf("%d %d\n", count_send, Bytesend);
                            fprintf(serial_send, "91,%d,%d,%d,%d,%d\n",(int)flagStriker,flagData1, flagData2, flagData3, flagData4);
                            }
						tempo_x = striker_3.coor_x[1];
						tempo_y = striker_3.coor_y[1];
					} else {
						if(count_send % counter_S2_2 == 0){
							fprintf(serial_send, "13,%d,%d,%d,%d,%d\n", (int)striker_2.x_mk_pos, (int)striker_2.y_mk_pos, (int)tempo_x, (int)tempo_y,flagKirim);
							fprintf(serial_send, "90,%d,%d,%d,%d,%d\n", dummypos1, dummypos2, dummypos3, dummypos4, timer3);
							// int Bytesend = fprintf(serial_send, "90,%d,%d,%d,%d,%d\n", dummypos1, dummypos2, dummypos3, dummypos4, timer3);
							// printf("%d %d\n", count_send, Bytesend);
                            fprintf(serial_send, "91,%d,%d,%d,%d,%d\n",(int)flagStriker,flagData1, flagData2, flagData3, flagData4);
...
//					fprintf(serial_send, "17,%d,%d\n", (int)striker_2.angle_obs, (int)striker_2.jarak_obs);
				...
				/*else if (TEAM == MAGENTA)
				{
					...
					if(count_send % counter_S2_1 == 0)fprintf(serial_send, "%d,%d,%d,%d,0\n", Header, Data1, Data2, mode_formasi[1]);					//Send Odometry Field To Robot
					if(count_ > 15){
						if(count_send % counter_S2_2 == 0)fprintf(serial_send, "13,%d,%d,%d,%d,%d\n", (int)striker_2.x_mk_pos, (int)striker_2.y_mk_pos, (int)striker_3.x_or[1], (int)striker_3.y_or[1],flagKirim);
						tempo_x = striker_3.x_or[1];
						tempo_y = striker_3.y_or[1];
					} else {
						if(count_send % counter_S2_2 == 0)fprintf(serial_send, "13,%d,%d,%d,%d,%d\n", (int)striker_2.x_mk_pos, (int)striker_2.y_mk_pos, (int)tempo_x, (int)tempo_y, flagKirim);
					}
					if(count_send % counter_S2_3 == 0)fprintf(serial_send, "11,%d,%d,%d,%d\n", (int)striker_3.distance_ball[0], (int)striker_3.angle_ball[1], (int)striker_3.x_ball[1], (int)striker_3.y_ball[1]);
				...
				if (TEAM == CYAN)
				{
					...
					if(count_ > 15){
						if(count_send % counter_S3_2 == 0){
							fprintf(serial_send, "13,%d,%d,%d,%d,%d\n",(int)striker_3.x_mk_pos, (int)striker_3.y_mk_pos,(int)striker_2.coor_x[1], (int)striker_2.coor_y[1],flagKirim);
							fprintf(serial_send, "90,%d,%d,%d,%d,%d\n", dummypos1, dummypos2, dummypos3, dummypos4, timer2);
                            fprintf(serial_send, "91,%d,%d,%d,%d,%d\n",(int)flagStriker,flagData1, flagData2, flagData3, flagData4);
                            }
						...
						if(count_send % counter_S3_2 == 0){
							fprintf(serial_send, "13,%d,%d,%d,%d,%d\n",(int)striker_3.x_mk_pos, (int)striker_3.y_mk_pos, (int)tempo_x, (int)tempo_y,flagKirim);
							fprintf(serial_send, "90,%d,%d,%d,%d,%d\n", dummypos1, dummypos2, dummypos3, dummypos4, timer2);
                            fprintf(serial_send, "91,%d,%d,%d,%d,%d\n",(int)flagStriker,flagData1, flagData2, flagData3, flagData4);
                            }
					}
...
//				striker_3.y_ball[1] = striker_3.y_or[2];
			...
			if (PROGRAM_UNTUK == STRIKER_2)
			{
				sprintf(buff_send, "111,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n", striker_2.x_or[1], striker_2.y_or[1], (int)striker_2.distance_ball[1], (int)striker_2.angle_ball[0], striker_2.x_or[2], striker_2.y_or[2], striker_2.yaw[1], (int)striker_2.x_mk_pos, (int)striker_2.y_mk_pos,striker_2.Volt[1], striker_2.timer);
				sendto(s, buff_send, strlen(buff_send), 0, (struct sockaddr *) &si_other, slen);
				memset(buff_send,'\0', sizeof(buff_send));	
			}
			else if (PROGRAM_UNTUK == STRIKER_3)
			{
				sprintf(buff_send, "222,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n", striker_3.x_or[1], striker_3.y_or[1], (int)striker_3.distance_ball[1], (int)striker_3.angle_ball[0], striker_3.x_or[2], striker_3.y_or[2], striker_3.yaw[1], (int)striker_3.x_mk_pos, (int)striker_3.y_mk_pos, striker_3.Volt[1], striker_3.timer);
				sendto(s, buff_send, strlen(buff_send), 0, (struct sockaddr *) &si_other, slen);
				memset(buff_send,'\0', sizeof(buff_send));	
			}
			else if (PROGRAM_UNTUK == KEEPER)
		...
		printf("\n%d. UDP_CLIENT %s -- RefBox = %d,%d,%d -- %s -- Team = %s -- mode = %d,%d -- %d,%d,%d,%d", count, SERVER, Header, Data1, Data2, rcvString, "CYAN", flagKirim, flagStriker, flagData1, flagData2, flagData3, flagData4);
		...
		if (PROGRAM_UNTUK == STRIKER_2)
		{
			printf("\n   STRIKER 2 --> Jarak,Sudut Bola = %d,%d -- Koordinat = %d,%d -- Positioning = %d,%d -- Dummy = %d,%d,%d,%d -- YAW = %d -- TIMER = %d", (int)striker_2.distance_ball[1], (int)striker_2.angle_ball[0], (int)striker_2.x_or[1], (int)striker_2.y_or[1], (int)striker_2.coor_y[1], (int)striker_2.coor_x[1], dummypos1, dummypos2, dummypos3, dummypos4,/* striker_2.x_ball[0], striker_2.y_ball[0],*/ striker_2.yaw[1], timer2);
			printf("\n   STRIKER 3 --> Jarak.Sudut Bola = %d,%d -- Koordinat = %d,%d -- Positioning = %d,%d -- Dummy = %d,%d,%d,%d -- YAW = %d -- TIMER = %d\n", (int)striker_3.distance_ball[0], (int)striker_3.angle_ball[0], (int)striker_3.coor_x[1], (int)striker_3.coor_y[1], (int)striker_3.coor_y[1], (int)striker_3.coor_x[1], dummypos1, dummypos2, dummypos3, dummypos4, striker_3.yaw[0], timer3);
//				printf("\n   A[%d],B[%d],C[%d],D[%d] = %.2f | %.2f | %.2f | %.2f\n", 0, 0, 0, 0, pengali_A[0], pengali_B[0], penambah_C[0],penambah_D[0]);
				printf("   FORMASI = %d\n", mode_formasi[1]);
		}
		if (PROGRAM_UNTUK == STRIKER_3)
		{
			printf("\n   STRIKER 2 --> Jarak,Sudut Bola = %d,%d -- Koordinat = %d,%d -- Positioning = %d,%d -- Dummy = %d,%d,%d,%d -- YAW = %d -- TIMER = %d", (int)striker_2.distance_ball[0], (int)striker_2.angle_ball[1], (int)striker_2.coor_x[1], (int)striker_2.coor_y[1], (int)striker_2.coor_y[1], (int)striker_2.coor_x[1], dummypos1, dummypos2, dummypos3, dummypos4, striker_2.yaw[0], timer2);
			printf("\n   STRIKER 3 --> Jarak,Sudut Bola = %d,%d -- Koordinat = %d,%d -- Positioning = %d,%d -- Dummy = %d,%d,%d,%d -- YAW = %d -- TIMER = %d\n", (int)striker_3.distance_ball[1], (int)striker_3.angle_ball[0], (int)striker_3.x_or[1], (int)striker_3.y_or[1], (int)striker_3.coor_y[1], (int)striker_3.coor_x[1], dummypos1, dummypos2, dummypos3, dummypos4,/* (int)striker_3.x_ball[0], (int)striker_3.y_ball[0],*/ (int)striker_3.yaw[1], timer3);
