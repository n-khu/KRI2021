...
int flag_switch = 1; // switch posisi robot
...
char *strikerMain; //= "Hades";
char *strikerAssist; //= "Ares ";
...
Mat fields, lapangan, newfields, symbol, newsymbol, simbol, lambang, flagStriker;
...
void print_striker(Mat& img, const string& name)
{
	Size textSize = getTextSize(name, fontFace, fontscale_striker, thickness, &baseline);
	baseline += thickness / thickness-5;
	Point textOrg((img.cols - textSize.width)/4, (img.rows + textSize.height)/2);
	rectangle(img, Point(30+170,40+301), Point(30+245,40+341), Scalar(100,0,255), CV_FILLED);
	putText(img, name, textOrg, fontFace, fontScale, Scalar::all(0), thickness, CV_AA);
}
...
int main(int argc, char **argv)
{
	...
	simbol.setTo(Scalar(0,0,0));

 	symbol = Mat::zeros(200,240,CV_32FC3);
	// Mat place = Mat::zeros(newWidth,newHeight,CV_32FC3);
	Mat rob = Mat::zeros(400,480,CV_32FC3);
	Mat formation = Mat::zeros(130,720,CV_32FC3);
	Mat Reset = Mat::zeros(240,720,CV_32FC3);

	// rectangle(place, Point(0,0), Point(900,1200), Scalar(255,255,255), CV_FILLED);
	// line(place, Point(0,5), Point(900,1200), CV_RGB(100,50,250),5,8,0);

	resize(fields, newfields, cvSize(690,483));
	newfields.convertTo(lapangan, CV_32FC3, 1/255.0);
	resize(simbol, newsymbol, cvSize(newWidth-980,newHeight/3));
	newsymbol.convertTo(lambang, CV_32FC3, 1/255.0);

//	lapangan.copyTo(result(Rect(13, 13, lapangan.cols, lapangan.rows)));

	///--------------------- UI STRIKER MODE ------------------------------------------// sini

	// putText(lambang, format("MAIN : %s", strikerMain), Point(15,35+4+40+30), FONT_HERSHEY_SIMPLEX, 0.8, Scalar(255,255,255), 1);
	// putText(lambang, format("ASSIST : %s", strikerAssist), Point(15,35+4+70+30), FONT_HERSHEY_SIMPLEX, 0.8, Scalar(255,255,255), 1);

	// putText(lambang, format("HADES "), Point(15,35+4+30), FONT_HERSHEY_SIMPLEX, 0.6, Scalar(255,255,255), 2);
	// putText(lambang, format("ARES "), Point(15,35+4+90), FONT_HERSHEY_SIMPLEX, 0.6, Scalar(255,255,255), 2); 	 

	rectangle(lambang, Point(0+5,15), Point(220-5,80), Scalar(255,255,255), CV_FILLED); //header
	rectangle(lambang, Point(0+5+5,15+5), Point(220-5-5,80-5), Scalar(0,0,0), CV_FILLED); //header
	putText(lambang, format("MODE"), Point(15+50,35+4+20), FONT_HERSHEY_SIMPLEX, 1, Scalar(0,255,0), 2); 	

	rectangle(lambang, Point(0+5,216-60), Point(220-5,216-5), Scalar(0,0,255), CV_FILLED); //luar button
	rectangle(lambang, Point(0+5+5,216-60+5), Point(220-5-5-100,216-5-5), Scalar(0,255,255), CV_FILLED); //button kiri
	rectangle(lambang, Point(0+5+5+100,216-60+5), Point(220-5-5,216-5-5), Scalar(255,255,0), CV_FILLED); //button kanan
	putText(lambang, format("H"), Point((220/2)-60,216-(54/2)), FONT_HERSHEY_SIMPLEX, 0.8, Scalar(0,0,0), 2);
	putText(lambang, format("A"), Point((220/2)+45,216-(54/2)), FONT_HERSHEY_SIMPLEX, 0.8, Scalar(0,0,0), 2);
	...
	putText(formation, format("ROBOT MODE"), Point(5,105), FONT_HERSHEY_PLAIN, 1.5, CV_RGB(255,255,255), 2);
...
//		if(ch != -1) printf("%c", ch);
		switch(ch){
				...
				case 'm' :
					flag_switch++;
					if (flag_switch == 3) flag_switch = 1; 
		}
...
//			}
			...
			putText(image_temp4, format("%d", flag_switch), Point(5+67,40+287), FONT_HERSHEY_PLAIN, 0.9, CV_RGB(255,255,255), 1);//flag_switch
...
//			ellipse(image_temp1, Point((int)keeper.yaw_x, (int)keeper.yaw_y), Size(4,4), 0,0,360, Scalar(120,0,255), CV_FILLED); //YAW
			...
			if (pressbut == true)
			{
				...
				//-------------------------------STRIKER MODE START---------------------------//sini

				if (button_x[1] >= 970 && button_x[1] <= 1069 && button_y[1] >= 165 && button_y[1] <= 205) //button kiri, mode hades
				{
					flag_switch = 0;
					strikerMain = "";
					strikerAssist = "";
				...
				if (button_x[1] >= 1070 && button_x[1] <= 1170 && button_y[1] >= 165 && button_y[1] <= 205) //button kanan, mode ares
				{
					flag_switch = 1;
					strikerMain = "";
					strikerAssist = "";
				...
				putText(image_temp4, format("%d", flag_switch), Point(15,35+4+40+30), FONT_HERSHEY_PLAIN, 0.9, CV_RGB(255,255,255), 1);
				// putText(lambang, format("MAIN : %s", strikerMain), Point(15,35+4+40+30), FONT_HERSHEY_SIMPLEX, 0.8, Scalar(255,255,255), 1);
				// putText(lambang, format("ASSIST : %s", strikerAssist), Point(15,35+4+70+30), FONT_HERSHEY_SIMPLEX, 0.8, Scalar(255,255,255), 1);
...
////			/*%d,%d,%d,%d,%d*/
			...
			// ref1, ref2, ref3, (int)striker_3.coor_x[0], (int)striker_3.coor_y[0], (int)striker_2.coor_x[0], (int)striker_2.coor_y[0], (int)flag_wifi, (int)striker_3.coor_x[4], (int)striker_3.coor_y[4], 
			// (int)striker_2.coor_x[4], (int)striker_2.coor_y[4],dummy_pos1, dummy_pos2, dummy_pos3, dummy_pos4, (int)striker_2.yaw, (int)striker_3.yaw, (int)striker_2.dist_ball, (int)striker_3.dist_ball, 
			// striker_3.reset_x[1], striker_3.reset_y[1], striker_2.reset_x[1], striker_2.reset_y[1], TEAM, mode_marking, mode_formasi, striker_3.x_cm[1], striker_3.y_cm[1], striker_2.x_cm[1], striker_2.y_cm[1], striker_2.timer, striker_3.timer);

			sprintf(message,"%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,1,1,%d,%d,%d,%d,%d,%d,%d\n", //35 data(33 int+2 flag)
			ref1, ref2, ref3, (int)striker_3.coor_x[0], (int)striker_3.coor_y[0], (int)striker_2.coor_x[0], (int)striker_2.coor_y[0], (int)flag_wifi, (int)striker_3.coor_x[4], (int)striker_3.coor_y[4], 
			(int)striker_2.coor_x[4], (int)striker_2.coor_y[4],dummy_pos1, dummy_pos2, dummy_pos3, dummy_pos4, (int)striker_2.yaw, (int)striker_3.yaw, (int)striker_2.dist_ball, (int)striker_3.dist_ball, 
			striker_3.reset_x[1], striker_3.reset_y[1], striker_2.reset_x[1], striker_2.reset_y[1], TEAM, mode_marking, striker_3.x_cm[1], striker_3.y_cm[1], striker_2.x_cm[1], striker_2.y_cm[1], striker_2.timer, striker_3.timer, flag_switch);
