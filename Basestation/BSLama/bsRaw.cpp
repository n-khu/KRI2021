...
int Robot_B = 0 ;
int Robot_A = 0 ;
int T = 0, V = 0, W = 0, X = 0,Y = 0, Z = 0, U = 0, YAW = 0, X_CM = 0, Y_CM = 0, VOLT = 0;
int A,B,C,D;
...
void Parsing_data2(char input[], int &out1, int &out2, int &out3, int &out4, int &out5,int &out6,int &out7, int &out8,int &out9,int &out10,int &out11)
{
    pry2 = strtok(input, ",");
    while (pry2 != 0 && pcountss <= 11)
    {
...
//	lapangan.copyTo(result(Rect(13, 13, lapangan.cols, lapangan.rows)));
	...
	putText(formation, format("FORMATION"), Point(5,40), FONT_HERSHEY_PLAIN, 1.5, CV_RGB(255,255,255), 2);
	...
	putText(formation, format("A"), Point(240+55,49), FONT_HERSHEY_PLAIN, 3, Scalar(0,0,0), 2);
	putText(formation, format("B"), Point(240+145,49), FONT_HERSHEY_PLAIN, 3, Scalar(0,0,0), 2);
	putText(formation, format("C"), Point(240+232,49), FONT_HERSHEY_PLAIN, 3, Scalar(0,0,0), 2);
	putText(formation, format("D"), Point(240+325,49), FONT_HERSHEY_PLAIN, 3, Scalar(0,0,0), 2);
...
//		if (count%2 != 0)// && count <= 14)
		for(int a = 0;a < 10;a++)
		{
			...
			Parsing_data2(buf, T, U, V, W, X, Y, Z, YAW, Y_CM, X_CM, VOLT);
...
//			}
			...
			putText(image_temp4, format("%d,%d", dummy_pos1, dummy_pos2), Point(5+67,40+231), FONT_HERSHEY_PLAIN, 0.9, CV_RGB(255,255,255), 1);//posisi dummy1
			putText(image_temp4, format("%d,%d", dummy_pos1, dummy_pos2), Point(5+227,40+231), FONT_HERSHEY_PLAIN, 0.9, CV_RGB(255,255,255), 1);//posisi dummy2
			putText(image_temp4, format("%d,%d", keeper.x_ball[0], keeper.y_ball[0]), Point(5+387,40+231), FONT_HERSHEY_PLAIN, 0.9, CV_RGB(255,255,255), 1);    //JARAK BOLA
...
//			ellipse(image_temp1, Point((int)keeper.yaw_x, (int)keeper.yaw_y), Size(4,4), 0,0,360, Scalar(120,0,255), CV_FILLED); //YAW
			...
			if (pressbut == true)
			{
				...
				else if (button_x[1] >= 805 && button_x[1] <= 875 && button_y[1] >= 135 && button_y[1] <= 195) { mode = "rst"; mode_marking = 8; print_mode(rob, mode); dummy_pos1=0; dummy_pos2=0;} //reset posisi dummy
				else if (button_x[1] >= 885 && button_x[1] <= 955 && button_y[1] >= 5 && button_y[1] <= 65) { mode = "6"; mode_marking = 3; print_mode(rob, mode); dummy_pos1=6;}
				else if (button_x[1] >= 885 && button_x[1] <= 955 && button_y[1] >= 70 && button_y[1] <= 130) { mode = "5"; mode_marking = 6; print_mode(rob, mode); dummy_pos1=5;}
				else if (button_x[1] >= 885 && button_x[1] <= 955 && button_y[1] >= 135 && button_y[1] <= 195) { mode = "4"; mode_marking = 9; print_mode(rob, mode); dummy_pos1=4;}
				...
				else if (button_x[1] >= 280 && button_x[1] <= 340 && button_y[1] >= 505 && button_y[1] <= 550) { mode = "A"; mode_formasi = 1; print_formasi(formation, mode, mode_formasi); }
				else if (button_x[1] >= 370 && button_x[1] <= 430 && button_y[1] >= 505 && button_y[1] <= 550) { mode = "B"; mode_formasi = 2; print_formasi(formation, mode, mode_formasi); }
				else if (button_x[1] >= 460 && button_x[1] <= 520 && button_y[1] >= 505 && button_y[1] <= 550) { mode = "C"; mode_formasi = 3; print_formasi(formation, mode, mode_formasi); }
				else if (button_x[1] >= 550 && button_x[1] <= 610 && button_y[1] >= 505 && button_y[1] <= 550) { mode = "D"; mode_formasi = 4; print_formasi(formation, mode, mode_formasi); }
				else 
				{ 
					mode = mode;
					mode_marking = 0; 
...
////			/*%d,%d,%d,%d,%d*/
			...
			sprintf(message,"%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n", 
			...
			(int)striker_2.coor_x[4], (int)striker_2.coor_y[4],dummy_pos1, dummy_pos2, (int)striker_2.yaw, (int)striker_3.yaw, (int)striker_2.dist_ball, (int)striker_3.dist_ball, 
			striker_3.reset_x[1], striker_3.reset_y[1], striker_2.reset_x[1], striker_2.reset_y[1], TEAM, mode_marking, striker_3.x_ball[0], 
			striker_3.y_ball[0], mode_formasi, striker_3.x_cm[1], striker_3.y_cm[1], striker_2.x_cm[1], striker_2.y_cm[1]);
//			
